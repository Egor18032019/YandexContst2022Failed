...
//                quickSort(storage, 0, storage.size() - 1, colum);
    ...
    public static void bubbleSort(List<int[]> sortArr, int colum) {
        for (int i = 0; i < sortArr.size() - 1; i++) {
            for (int j = 0; j < sortArr.size() - i - 1; j++) {
                if (sortArr.get(j + 1)[colum] < sortArr.get(j)[colum]) {
                    int[] swap = sortArr.get(j);
                    int[] swapJ = sortArr.get(j + 1);
                    sortArr.remove(j);
                    sortArr.add(j, swap);
                    sortArr.remove(j + 1);
                    sortArr.add(j, swapJ);
